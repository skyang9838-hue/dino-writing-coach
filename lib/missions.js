...
// The seed for a criterion that is already met but got picked anyway, because
// every round hands out exactly two missions and there was nothing weaker left
// to work on. It lives on the criterion (lib/curriculum.js) rather than in a
// table here, so a unit brings its own wording with it.
  ...
  const refinementTargets = assessments
    ...
    .map((assessment) => {
      ...
      const target = {
        ...
        priority: criterion.priority,
        refinement: true,
        missionSeed: criterion.refinementSeed,
      }
      return {
        ...target,
        repeatedTwice: wasUsedInBothRecentRounds(target, priorRounds),
