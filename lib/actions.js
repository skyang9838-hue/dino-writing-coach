...
// Shared by both the rule-based guard (lib/guard.js) and the AI's own
// meaningless-content judgment (lib/coaching.js) — either can flag a round.
...
async function flagRoundAsNonsense(submission, writing, reason) {
...
export async function requestCoaching(submissionId, writing) {
  ...
  if (guard.flagged) {
    return flagRoundAsNonsense(submission, writing, guard.reason)
  }
  ...
  if (result.meaningless) {
    return flagRoundAsNonsense(submission, writing, 'nonsense')
  }
