...
import { INTERVIEW_REPORT_GENRE, getUnitById } from './curriculum.js'
import {
...
// flag a round. `attainment` is passed explicitly so every flagged round preserves
// the last meaningful cumulative baseline.
...
async function runInterviewReportRound(submission, writing) {
  let result
  try {
    result = await getInterviewReportFeedback({
  ...
  if (result.meaningless) {
    return flagRound(submission, writing, 'nonsense', submission.attainment)
  }
...
async function runCoachingRound(submission, writing) {
  if (submission.activity.genre === INTERVIEW_REPORT_GENRE) {
    return runInterviewReportRound(submission, writing)
  }
...
export async function requestCoaching(submissionId, writing) {
  ...
  if (guard.flagged) {
    return flagRound(submission, writing, guard.reason, submission.attainment)
  }
