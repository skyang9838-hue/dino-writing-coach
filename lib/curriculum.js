...
export const GENRE_COACHING_GUIDANCE = {
  ...
  기행문: '여행하며 다닌 곳(여정), 보고 들은 것(견문), 생각하거나 느낀 것(감상)이 모두 드러나는지도 함께 봐줘.',
}
...
export const GRADE6_SEMESTER2_UNITS = [
  ...
  {
    id: 'g6s2-supplement',
    // 교육과정 성취기준에는 있는데 교과서에 해당 단원이 없다. 교사가 그래도
    // 가르쳐야 해서 교육과정에서 부르는 이름 그대로 '보완단원'이다. 매체
    // 단원과 마찬가지로 번호가 없어 unitLabel이 번호 자리에 들어간다.
    unitLabel: '보완단원',
    icon: '🧳',
    title: '여정, 견문, 감상이 드러나는 기행문 쓰기',
    description: '여행하며 다닌 곳과 보고 들은 것, 그때 생각하고 느낀 것을 글로 써요.',
    genre: '기행문',
    recommendedLength: 600,

    // 6국03-03이 첫 청크(기행문의 특성), 6국03-05가 둘째 청크(쓰기 과정과
    // 고쳐쓰기)에 대응한다. 둘째 청크는 항목이 전부 교사 몫이라 getAiRubrics가
    // 통째로 걸러 낸다 — 디노는 6국03-05가 있다는 사실 자체를 모른다.
    standards: [
      {
        code: '6국03-03',
    ...
    assessmentGuidance: [
      '여기서 여행은 수학여행, 현장체험학습, 가족여행은 물론 박물관 견학이나 동네 나들이처럼 학생이 직접 체험한 일이면 모두 해당해. 멀리 다녀온 여행만 인정하지 마.',
    ],

    missionGuidance: {
      grounding: '학생 글에 실제로 나온 내용을 짚되 학생이 가지 않은 곳이나 보지 않은 것을 만들어내지 마.',
      selfFill: '질문이나 단서는 줄 수 있지만 학생이 무엇을 보고 느꼈는지 스스로 떠올려 채우게 해.',
    },
    ...
    chunks: [
      {
        id: 'travel-writing',
        label: '기행문의 특성',
        // 항목 차례는 글의 흐름(처음 → 가운데 → 끝)이다. 보드가 이 차례대로
        // 그리기 때문에 교사가 학생 글을 따라 읽으며 짚을 수 있다. 미션을 무엇부터
        // 낼지는 배열 순서가 아니라 priority가 정하는데, 그쪽은 기행문의 정체인
        // 여정·견문·감상을 앞에 뒀다. 둘이 다른 건 일부러다.
        criteria: [
          {
            id: 'trip-reason',
            evaluator: 'ai',
            label: '글 처음 부분에 여행을 떠난 까닭과 여행에 대한 기대가 드러나 있다.',
            shortLabel: '떠난 까닭과 기대',
            priority: 40,
            statuses: {
              met: '왜 그곳에 가게 됐는지와 가기 전에 무엇을 기대했는지가 둘 다 드러난다.',
              partial: '떠난 까닭과 기대 중 하나만 드러난다.',
              unmet: '떠난 까닭도 기대도 찾을 수 없다.',
            },
            missionSeed: '여행을 떠난 까닭과 가기 전에 기대했던 것 가운데 빠진 쪽을 글 앞부분에 밝히도록 안내',
            refinementSeed: '이미 밝힌 까닭과 기대가 뒤에 나오는 여행 이야기와 더 분명히 이어지도록 다듬게 안내',
            fallbackMissionTitle: '떠난 까닭 밝히기',
          },
          {
            id: 'itinerary',
            evaluator: 'ai',
            label: '여행하며 다닌 곳을 나타내는 여정이 있다.',
            shortLabel: '여정이 드러난다',
            priority: 10,
            statuses: {
              met: '어디에 갔는지와 어떤 차례로 옮겨 다녔는지가 글을 따라가며 드러난다.',
              partial: '간 곳이 나오기는 하나 한 군데뿐이거나 옮겨 다닌 차례를 알 수 없다.',
              unmet: '어디에 갔는지 찾을 수 없다.',
            },
            missionSeed: '여행하며 들른 곳을 하나 더 떠올려, 어디에서 어디로 옮겨 갔는지 차례가 드러나게 쓰도록 안내',
            refinementSeed: '이미 쓴 여정에 옮겨 간 차례가 더 또렷하게 드러나도록 이어 주는 말을 다듬게 안내',
            fallbackMissionTitle: '다닌 곳 이어 쓰기',
            // 여정과 견문은 학생 글에서 한 문장에 섞여 나온다("경복궁에 가서
            // 근정전을 봤다"). 둘을 가르는 선을 판정 쪽에 못박아 둔다.
            assessmentNotes: [
              '여정은 어디에 갔는지와 어떤 차례로 옮겨 다녔는지야. 장소가 한 군데만 나오고 옮겨 다닌 자취가 없으면 partial이야.',
              '그곳에서 무엇을 보았는지는 여정이 아니라 견문이야. 여정은 장소와 이동만 본다.',
            ],
          },
          {
            id: 'observation',
            evaluator: 'ai',
            label: '여행하며 보고 들은 것을 나타내는 견문이 있다.',
            shortLabel: '견문이 드러난다',
            priority: 20,
            statuses: {
              met: '그곳에서 직접 보거나 들은 것이 무엇인지 떠올릴 수 있게 그려진다.',
              partial: '보고 들었다고 말하기는 하나 “구경했다”처럼 무엇을 봤는지 알 수 없다.',
              unmet: '보고 들은 것을 찾을 수 없다.',
            },
            missionSeed: '들른 곳 가운데 하나를 골라, 거기에서 눈으로 본 것이나 귀로 들은 것을 한 가지 덧붙이도록 안내',
            refinementSeed: '이미 쓴 견문에 그때 본 장면이 더 또렷해지도록 꾸미는 말을 다듬게 안내',
            fallbackMissionTitle: '본 것 덧붙이기',
            assessmentNotes: [
              '견문은 그곳에서 보거나 들은 것이야. 장소 이름만 있고 무엇을 봤는지 없으면 그건 견문이 아니라 여정이야.',
              '“구경했다”, “많이 봤다”처럼 무엇을 봤는지 알 수 없는 말만 있으면 partial이야.',
            ],
          },
          {
            id: 'impression',
            evaluator: 'ai',
            label: '여행하며 생각하거나 느낀 것이 드러나 있다.',
            shortLabel: '감상이 드러난다',
            priority: 30,
            statuses: {
              met: '보고 들은 것에 대해 그때 어떤 생각이 들었고 어떤 마음이었는지가 드러난다.',
              partial: '“재미있었다”, “좋았다”처럼 까닭이 없는 말만 있다.',
              unmet: '생각이나 느낌을 찾을 수 없다.',
            },
            missionSeed: '보고 들은 것 하나를 골라, 그때 어떤 생각이 들었는지 까닭과 함께 쓰도록 안내',
            refinementSeed: '이미 쓴 생각과 느낌이 바로 앞에 나온 견문과 더 분명히 맞물리도록 다듬게 안내',
            fallbackMissionTitle: '느낀 까닭 쓰기',
            assessmentNotes: [
              '“재미있었다”, “좋았다”, “신기했다”처럼 왜 그런지가 없는 말만 있으면 impression은 partial이야.',
            ],
          },
          {
            // 판정은 선생님 몫이다 — '생생한가'는 사람이 볼 일이라. 다만 마무리에
            // 느낌을 더 쓰라는 조언은 디노가 할 수 있어서 teacher-ai-feedback으로
            // 뒀다. 보드에는 —로 나가고(showsAiVerdict) 수정미션에는 들어간다.
            id: 'overall-feeling',
            evaluator: 'teacher-ai-feedback',
            label: '글 끝 부분에 여행하고 난 뒤 전체적인 느낌이 생생히 드러나 있다.',
            shortLabel: '전체적인 느낌이 생생하다',
            priority: 50,
            statuses: {
              met: '여행을 마치고 난 뒤의 느낌이 글 끝에서 구체적인 말로 그려진다.',
              partial: '마무리에 느낌이 있기는 하나 “좋은 여행이었다” 정도로 뭉뚱그려져 있다.',
              unmet: '여행을 마친 뒤의 느낌을 찾을 수 없다.',
            },
            missionSeed: '글 마지막에 여행 전체를 돌아본 느낌을 자기 말로 한두 문장 덧붙이도록 안내',
            refinementSeed: '이미 쓴 마무리 느낌이 앞서 쓴 여정·견문과 더 이어지도록 다듬게 안내',
            fallbackMissionTitle: '마무리 느낌 쓰기',
          },
          {
            id: 'future-plan',
            evaluator: 'ai',
            label: '앞으로의 계획이나 각오가 잘 드러나 있다.',
            shortLabel: '앞으로의 계획·각오',
            priority: 60,
            statuses: {
              met: '이 여행을 하고 나서 앞으로 무엇을 하고 싶은지나 어떻게 하겠다는 다짐이 드러난다.',
              partial: '앞으로의 이야기가 있으나 “또 가고 싶다” 정도에 그친다.',
              unmet: '앞으로의 계획이나 각오를 찾을 수 없다.',
            },
            missionSeed: '이 여행을 하고 나서 앞으로 해 보고 싶은 일이나 다짐을 한 가지 글 끝에 쓰도록 안내',
            refinementSeed: '이미 쓴 계획이나 각오에 이번 여행에서 그렇게 마음먹게 된 까닭이 붙도록 다듬게 안내',
            fallbackMissionTitle: '앞으로의 다짐 쓰기',
          },
        ],
      },
      {
        // 여기 둘은 판정도 조언도 전부 선생님 몫이다. 쓰기 과정을 점검했는지도,
        // 고쳐쓰기 전후가 어떻게 달라졌는지도 완성된 글 하나만 봐서는 알 수 없다.
        // AI 판정 항목이 하나도 없어 getAiRubrics가 이 청크를 통째로 걸러 낸다.
        id: 'process-revision',
        label: '쓰기 과정과 고쳐쓰기',
        criteria: [
          {
            id: 'writing-process',
            evaluator: 'teacher',
            label: '쓰기 과정을 점검하고 조정하며 글을 썼다.',
            shortLabel: '쓰기 과정을 점검했는가?',
          },
          {
            id: 'coherence-revision',
            evaluator: 'teacher',
            label: '글 전체가 통일성 있도록 고쳐 썼다.',
            shortLabel: '통일성 있게 고쳐썼는가?',
          },
        ],
      },
    ],
