...
const missions = [
  ...
  },
  {
    ...
    title: '느낀 점 연결하기',
    instruction: '글 끝의 느낀 점을 본문의 면담 사실 하나와 연결해 보세요.',
...
describe('buildInterviewRoundState', () => {
  it('adds 10% only for each prior mission assessed as done', () => {
    ...
    expect(state.feedback).not.toHaveProperty('complete')
    expect(state.rounds.at(-1)).not.toHaveProperty('complete')
  })
  ...
  it('starts the first meaningful coaching round at 40 even when every rubric is met', () => {
    ...
    expect(state.feedback.missions).toHaveLength(2)
    expect(state.feedback).not.toHaveProperty('complete')
  })
