...
describe('INTERVIEW_ASSESSMENT_SCHEMA', () => {
  ...
  it('matches the unit criteria the pipeline actually judges', () => {
    ...
    expect(INTERVIEW_ASSESSMENT_SCHEMA.properties.criteria.minItems).toBe(criteria.length)
    expect(INTERVIEW_ASSESSMENT_SCHEMA.properties.criteria.maxItems).toBe(criteria.length)
  })
})
...
describe('Gemini responseSchema compatibility', () => {
  ...
  it.each([
    ['INTERVIEW_ASSESSMENT_SCHEMA', INTERVIEW_ASSESSMENT_SCHEMA],
    ['INTERVIEW_MISSION_SCHEMA', INTERVIEW_MISSION_SCHEMA],
...
describe('getInterviewReportFeedback', () => {
  ...
  it('still asks for a fresh assessment when there is no prior round to reuse', async () => {
    ...
    await getInterviewReportFeedback({
      topic: '면담 보고서',
      writing: '같은 글이다.',
