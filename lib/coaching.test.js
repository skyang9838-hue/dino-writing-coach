...
describe('INTERVIEW_MISSION_SCHEMA', () => {
  it('constrains mission rubric and criterion arrays to exact application IDs', () => {
    ...
    expect(itemProperties.rubricIds.items.enum).toEqual([
      'purpose-and-subject',
      'information',
      'structure',
    ])
    expect(itemProperties.criterionIds.items.enum).toEqual([
      'purpose',
      'interviewee',
      'new-fact',
      'fact-detail',
...
describe('getInterviewReportFeedback', () => {
  it('handles an unchanged revision without calling Gemini and marks prior missions not-done', async () => {
    const previousMissions = [
      {
        ...
        criterionIds: ['closing'],
        title: '느낀 점 덧붙이기',
        instruction: '글 끝에 면담 뒤 든 생각을 덧붙여 보세요.',
      ...
      },
      {
        ...
        title: '들은 까닭 연결하기',
        instruction: '새 사실 뒤에 면담에서 들은 까닭을 연결해 보세요.',
        criterion: '새 사실에 면담에서 들은 까닭이 연결되었는가',
      },
    ]
    const assessments = completeAssessments({ closing: 'unmet' })

    const result = await getInterviewReportFeedback({
      topic: '면담 보고서 쓰기',
      writing: '고치지 않은 글',
      previousWriting: '고치지 않은 글',
      previousMissions,
      priorRounds: [{
        writing: '고치지 않은 글',
        strength: '면담 대상을 밝혔어요.',
        assessments,
    ...
    expect(result.missions).toEqual(previousMissions)
  })
})
...
describe('sanitizeInterviewMissionResult', () => {
  ...
  it('uses server-selected target IDs instead of model-invented target metadata', () => {
    const selectedTargets = [
      {
        rubricIds: ['information'],
        criterionIds: ['new-fact', 'fact-detail'],
