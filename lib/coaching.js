...
export function sanitizeInterviewMissionResult(result) {
  ...
  return {
    ...
    missions: result.missions.map((mission) => ({
      ...
      instruction: typeof mission.instruction === 'string'
        ? mission.instruction.replace(/\s*예를\s*들어[,\s\S]*$/u, '').trim()
        : mission.instruction,
    })),
  }
}
...
async function callGeminiJson({
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    const rawResult = await requestGeminiJson({ prompt: currentPrompt, schema, temperature })
...
export async function getInterviewReportFeedback({
  ...
  const missionResult = await callGeminiJson({
    ...
    temperature: 0.7,
    normalize: sanitizeInterviewMissionResult,
