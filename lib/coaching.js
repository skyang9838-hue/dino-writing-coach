...
// Resolved once at module load. getAiRubrics strips the teacher-judged
// criteria, so nothing below this line — prompt, schema, mission targets —
  ...
  properties: {
    ...
    criteria: {
      ...
      items: {
        type: 'object',
        properties: {
          // Only one chunk reaches the AI: 상대와 매체 고려 is teacher-judged
          // and getAiRubrics drops it before the pipeline ever sees it.
          rubricId: {
            type: 'string',
            enum: ['interview-procedure'],
          },
          criterionId: {
            type: 'string',
            enum: ['purpose-and-target', 'preparation', 'learned-facts', 'reflection'],
          },
  ...
  properties: {
    strength: {
      ...
      properties: {
        rubricId: {
          type: 'string',
          enum: ['interview-procedure', 'effort'],
        },
        criterionId: {
          type: 'string',
          enum: ['purpose-and-target', 'preparation', 'learned-facts', 'reflection', 'effort'],
        },
        text: { type: 'string' },
      },
...
- 채점기준 4개는 현재 학생 글 전체를 기준으로 판정해. 고치기 전후의 변화는 지난 수정미션 수행 상태를 판정할 때만 사용해.
...
// impossible `opening: met`. The four criteria are independent of one another
// — none of them can contradict another — so there is nothing left to correct.
...
const INTERVIEW_FALLBACK_TITLE_BY_CRITERION = {
  'purpose-and-target': '면담 목적과 대상 밝히기',
  preparation: '면담 준비 과정 밝히기',
  'learned-facts': '새 사실에 설명 덧붙이기',
  reflection: '느낀 점에 까닭 덧붙이기',
}
