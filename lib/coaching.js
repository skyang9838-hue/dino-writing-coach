    ...
    missions: {
      ...
      items: {
        ...
        properties: {
          ...
          // 1-or-2 constraint is enforced in validateInterviewMissions instead.
...
export async function getInterviewReportFeedback({
  ...
  // 교사 보드에 있지도 않은 향상 ↑ 과 하락 ▼ 화살표가 떴다. 아래 priorMissions를
  // 코드로 확정하는 것과 같은 이유이고, 덤으로 Gemini 호출도 한 번 줄어든다.
  ...
  const assessmentResult = reusableAssessments
    ...
    : await callJson({
      prompt: buildInterviewAssessmentPrompt({
        topic,
        writing,
        previousWriting,
        previousMissions,
