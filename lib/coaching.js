  ...
  properties: {
    strength: {
      ...
      properties: {
        rubricId: {
          type: 'string',
          enum: ['purpose-and-subject', 'information', 'structure', 'effort'],
        },
        criterionId: {
          type: 'string',
          enum: ['purpose', 'interviewee', 'new-fact', 'fact-detail', 'opening', 'body', 'closing', 'effort'],
        },
        text: { type: 'string' },
      },
...
- 루브릭 7개는 현재 학생 글 전체를 기준으로 판정해. 고치기 전후의 변화는 지난 수정미션 수행 상태를 판정할 때만 사용해.
...
- 짧은 한 문장 안에 이유나 과정이 분명하면 fact-detail은 met이야. 문장이 짧다는 이유만으로 partial로 낮추지 마.
...
- 목적과 대상이 있어도 준비 행동이 하나도 없으면 opening은 반드시 partial이야.
...
- 느낀 점 뒤에 다시 면담 사실이 나오면 closing은 partial이야. 그 면담 사실이 글의 끝 문장이라는 이유로 closing을 met으로 판정하지 마.
...
export function normalizeInterviewAssessment(result) {
  if (!result || !Array.isArray(result.criteria)) return result
  const statuses = new Map(
    result.criteria.map((assessment) => [assessment.criterionId, assessment.status]),
  )
  const impossibleOpening =
    statuses.get('opening') === 'met'
    && (statuses.get('purpose') !== 'met' || statuses.get('interviewee') !== 'met')

  return {
    ...result,
    criteria: result.criteria.map((assessment) =>
      impossibleOpening && assessment.criterionId === 'opening'
...
const INTERVIEW_FALLBACK_TITLE_BY_CRITERION = {
  purpose: '면담 목적 분명히 밝히기',
  interviewee: '면담 대상 소개하기',
  'new-fact': '새로 안 사실 덧붙이기',
  'fact-detail': '새 사실에 설명 덧붙이기',
  opening: '면담 준비 과정 배치하기',
  body: '새로 안 사실 모아 쓰기',
  closing: '느낀 점에 까닭 덧붙이기',
}
...
export function sanitizeInterviewMissionResult(result, writing = '', selectedTargets = []) {
  if (!result || !Array.isArray(result.missions)) return result
  const normalizedWriting = writing.replace(/\s+/g, ' ')
  return {
    ...result,
    missions: result.missions.map((mission, index) => ({
      ...mission,
      title:
        typeof mission.title === 'string' &&
        VAGUE_MISSION_TITLE_PATTERN.test(mission.title) &&
        INTERVIEW_FALLBACK_TITLE_BY_CRITERION[
          selectedTargets[index]?.criterionIds?.at(-1)
        ]
          ? INTERVIEW_FALLBACK_TITLE_BY_CRITERION[
              selectedTargets[index].criterionIds.at(-1)
            ]
      ...
      ...(selectedTargets[index]
        ? {
            rubricIds: selectedTargets[index].rubricIds,
            criterionIds: selectedTargets[index].criterionIds,
          }
        : {}),
...
export function validateInterviewMissionResult(
  ...
  if (
    ...
    && (result.strength.rubricId !== 'effort' || result.strength.criterionId !== 'effort')
  ) {
...
export async function getInterviewReportFeedback({
  ...
  if (
    ...
    return {
      ...
      strength: {
        rubricId: strengthCriterion?.rubricId ?? 'effort',
        criterionId: strengthCriterion?.criterionId ?? 'effort',
  ...
  const assessmentResult = await callGeminiJson({
    ...
    temperature: 0,
    normalize: normalizeInterviewAssessment,
  ...
  const missionResult = await callGeminiJson({
    ...
    temperature: 0.7,
    maxAttempts: 5,
    normalize: (result) => sanitizeInterviewMissionResult(result, writing, selectedTargets),
