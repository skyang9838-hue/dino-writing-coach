...
describe('getMascotState', () => {
  ...
  it('celebrates continued growth above 100% without calling the writing perfect', () => {
    expect(getMascotState(100)).toMatchObject({ face: 'face-love' })
    expect(getMascotState(1000)).toMatchObject({ face: 'face-love' })
    expect(getMascotState(100).message).not.toContain('완벽')
    expect(getMascotState(1000).message).toContain('다음 미션')
  })

  it('falls back to the default face below 40%', () => {
