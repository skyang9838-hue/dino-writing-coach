...
describe('GRADE6_SEMESTER2_UNITS', () => {
  it('lists the semester 2 units in unit-number order', () => {
    expect(GRADE6_SEMESTER2_UNITS.map((unit) => unit.unitNumber)).toEqual([1, 2, 3, 5, 6])
  })
...
describe('unit achievement standards', () => {
  ...
  it('carries both standards for the 매체 unit', () => {
    expect(getUnitStandards('g6s2-unit3').map((standard) => standard.code)).toEqual([
      '6국06-04',
      '6국03-06',
    ])
  })
...
describe('getAiRubrics', () => {
  ...
  it.each([
    ...
    ['g6s2-unit3', ['participation', 'share-audience']],
  ...
  it('drops a chunk once its only criteria are teacher-judged', () => {
    ...
    expect(getAiRubrics('g6s2-unit3').map((rubric) => rubric.id)).toEqual(['media-reflection'])
  })
