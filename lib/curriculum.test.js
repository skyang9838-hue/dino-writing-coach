...
describe('GRADE6_SEMESTER2_UNITS', () => {
  it('lists the numbered semester 2 units in order', () => {
    expect(GRADE6_SEMESTER2_UNITS.map((unit) => unit.unitNumber)).toEqual([
      1,
      2,
      5,
      undefined,
  ...
  it('gives every unit either a number or a label to show in its place', () => {
    ...
    // 교과서에 없는 성취기준을 메우는 단원이라 번호가 없다.
    const supplement = getUnitById('g6s2-supplement')
    expect(supplement.unitLabel).toBe('보완단원')
    expect(supplement.unitNumber).toBeUndefined()
  })
...
describe('unit achievement standards', () => {
  ...
  // 보완단원은 기행문 성취기준에 고쳐쓰기 성취기준을 얹었다. 뒤쪽은 항목이
  // 전부 교사 몫이라 채점기준으로는 AI에게 가지 않지만, 보드에는 나와야 한다.
  it('carries both standards for the 보완 unit', () => {
    expect(getUnitStandards('g6s2-supplement').map((standard) => standard.code)).toEqual([
      '6국03-03',
      '6국03-05',
    ])
  })
...
describe('every unit with criteria', () => {
  ...
  it('covers the five units the curriculum notes describe', () => {
...
describe('getAiRubrics', () => {
  ...
  it.each([
    ...
    ['g6s2-supplement', ['writing-process', 'coherence-revision']],
  ...
  it('drops a chunk once its only criteria are teacher-judged', () => {
    ...
    // 보완단원도 마찬가지다. 쓰기 과정과 고쳐쓰기는 완성된 글 하나만 봐서는
    // 알 수 없어서, 그 성취기준이 통째로 AI 밖에 남는다.
    expect(getAiRubrics('g6s2-supplement').map((rubric) => rubric.id)).toEqual(['travel-writing'])
  })
