...
**2026-09-02 — 보완단원(기행문) 추가** (맨 아래 항목)
교과서에 단원이 없는 성취기준 `6국03-03`을 메우는 단원. 여정과 견문을 갈라 판정하게 하는 것이 관건이었다.

**2026-08-30~31 — 채점기준 구조 개편 + 단원 구동식 전환**
채점기준을 성취기준 → 청크 → 항목 구조로 바꾸고, 2학기 단원 5개를 넣어 AI 채점기준이 있는 단원이면 어느 단원이든 같은 파이프라인을 타게 했다. 보드 스크롤 컨트롤과 판정 기호 완화도 이때.

**2026-08-02 — 대시보드 end-to-end 검증 + 판정 안정성 수정**
...
## 2026-09-02 — 보완단원(기행문) 추가 (`1310727`)

### 왜 '보완단원'인가

`[6국03-03] 체험한 일에 대한 감상을 나타내는 글을 쓴다`는 교육과정 성취기준에는 있는데 **교과서에 해당 단원이 없다.** 교사는 그래도 가르쳐야 한다. 그래서 교육과정이 부르는 이름 그대로 단원명을 **보완단원**으로 두고, 활동을 "여정, 견문, 감상이 드러나는 기행문 쓰기"로 만들었다. 고쳐쓰기 성취기준 `6국03-05`도 함께 얹었다.

구조는 새로 만들 것이 없었다. 파이프라인이 이미 단원 구동식이라 `GRADE6_SEMESTER2_UNITS`에 객체 하나를 더하고 하드코딩된 목록 몇 개를 따라 고친 것이 전부다. **매체 단원(`g6s2-media`)이 정확한 선례였다** — 교육과정에 번호가 없어 `unitNumber` 대신 `unitLabel`이 카드의 번호 자리에 들어가고, 성취기준을 두 개 들고 있다는 점까지 같다.

### 채점기준 — 청크 2개 / 항목 8개

| 청크 | 성취기준 | 항목 | 주체 |
|---|---|---|---|
| 기행문의 특성 | 6국03-03 | 떠난 까닭과 기대 / **여정** / **견문** / **감상** / 전체적인 느낌 / 앞으로의 계획·각오 | `ai` 5 + `teacher-ai-feedback` 1 |
| 쓰기 과정과 고쳐쓰기 | 6국03-05 | 쓰기 과정 점검·조정 / 통일성 있게 고쳐쓰기 | `teacher` 2 |

뒤쪽 청크는 **통째로 AI 밖에 남는다.** 항목이 둘 다 `teacher`라 `getAiRubrics`가 청크째로 걸러 내기 때문이다 — 쓰기 과정을 점검했는지도, 고쳐쓰기로 무엇이 달라졌는지도 완성된 글 하나만 봐서는 알 수 없다. 별도 조치는 필요 없었고 방화벽이 그대로 처리했다.

`overall-feeling`("전체적인 느낌이 생생한가")만 `teacher-ai-feedback`이다. 생생한지는 사람이 볼 일이지만, 마무리에 느낌을 한 줄 더 쓰라는 조언은 디노가 할 수 있다.

항목 차례는 글의 흐름(처음 → 가운데 → 끝)이고, `priority`는 기행문의 정체인 여정·견문·감상을 앞에 뒀다. **보드가 읽는 차례와 미션이 나가는 차례가 다른 건 일부러다.**

`기행문`은 `GENRES`에 없던 장르라 코칭 가이던스(`GENRE_COACHING_GUIDANCE`)와 아이콘(`GENRE_ICONS`)까지 세 곳에 등록했다. 가이던스를 빠뜨리면 `curriculum.test.js`가 실패한다.

### 여정과 견문을 가르는 문제

이 단원의 유일한 실질적 위험이었다. 둘은 학생 글에서 한 문장에 섞여 나온다 — "경복궁에 가서 근정전을 봤다"는 여정이면서 견문이다. 매체 단원에서 "게임을 많이 한다"가 `met`으로 나왔던 것과 같은 종류라, 같은 방식으로 `assessmentNotes`에 선을 못박았다.

> 그곳에서 무엇을 보았는지는 여정이 아니라 견문이야. 여정은 장소와 이동만 본다.
> 장소 이름만 있고 무엇을 봤는지 없으면 그건 견문이 아니라 여정이야.

이걸 노린 스모크 케이스(`travel-places-only` — 박물관→남산→명동→청계천을 순서대로 나열하되 무엇을 봤는지는 없는 글)에서 **`itinerary` met / `observation` partial로 갈렸다.** 3회 모두.

### 검증

`npm test` **232개 통과**(224 → 232), `npm run lint` 무경고, `npm run build` 성공.

`npm run smoke -- --unit g6s2-supplement --runs 3` — 3케이스 × 3회 = **9회 · 실패 0 · 판정 흔들림 0 · 의도 일치 15/18**.

어긋난 3건은 모두 3회 내내 흔들리지 않았고, **정답을 지어낸 쪽이 틀렸을 가능성이 크다**:

| 기준 | 의도 | 판정 | 보기 |
|---|---|---|---|
| `trip-reason` ×2 | `unmet` | `partial` | "현장체험학습을 갔다", "할머니 댁에 갔다"를 떠난 까닭으로 읽었다 — 모델 쪽이 오히려 그럴듯하다 |
| `overall-feeling` | `partial` | `unmet` | 끝의 "재미있었다"를 느낌으로 안 쳤다. 반대로 엄격한 쪽 |

### 브라우저 실측 — 학생 흐름 3라운드 (2026-09-02)

로컬 dev 서버로 활동을 만들고 학생으로 참여해 **글 쓰기 → 코칭 → 미션대로 고치기 → 재코칭**을 세 바퀴 돌렸다. 첫 글은 일부러 여정만 촘촘하고 견문·감상이 빈 글(610자)로 넣었다.

| 회차 | 도달도 | 판정 변화 | 나온 미션 |
|---|---|---|---|
| 1차 | 40% | 견문·감상·계획 `?` | **본 것 덧붙이기**(`observation`) · **느낀 점과 까닭 쓰기**(`impression`) |
| 2차 | 50% | 견문 `?`→`✓` ↑ | **여행을 돌아본 느낌 덧붙이기**(`overall-feeling`) · 느낀 점 이유와 함께 쓰기 |
| 3차 | 70% | 감상·계획 `?`→`✓` ↑ | 이동 과정 묘사하기 · 불국사 탑 묘사 **다듬기** |

확인된 것:

- **`priority`가 의도대로 작동한다.** 여정이 이미 met이라 미션에서 빠지고 견문(20) → 감상(30) 순으로 잡혔다
- **도달도 규칙이 맞다.** 40% 고정 → 미션 done 하나당 +10%(2차는 1개만 done이라 50%, 3차는 2개 다 done이라 70%)
- **`refinementSeed`로 갈아탄다.** 3차에서 AI 기준이 전부 `✓`가 되자 미션 둘이 모두 "묘사하기 / 다듬기"로 바뀌었다 — 코칭에 끝이 없다는 규칙이 실제로 돈다
- **`teacher-ai-feedback`이 설계대로다.** `overall-feeling`이 2차에서 **미션으로는 나갔는데** 보드 판정은 `—`로 감춰져 있다. `getAiRubrics`(통과)와 `showsAiVerdict`(차단) 두 관문이 각각 제 일을 했다
- **교사 방화벽 실측.** `writing-process`·`coherence-revision`은 3라운드 내내 판정에 등장하지 않고 `—` + `교사` 배지로만 나왔다
- 보드에 청크 2개가 「기행문의 특성」/「쓰기 과정과 고쳐쓰기」로 갈려 그려지고, 성취기준 `[6국03-03] [6국03-05]`가 상단 `<details>`에 함께 나온다. 범례에 AI·교사·교사·AI 세 배지가 모두 뜬다(이 단원이 셋을 다 쓰는 첫 단원이다)
- 활동 생성 화면: 카드 6개, 보완단원이 맨 뒤, 번호 자리에 `보완단원`, 아이콘 🧳, 선택 시 목표 글자 수가 600으로 자동 이동

⚠️ **이 검증으로 프로덕션 DB에 테스트 데이터가 남았다** — 활동(참여 코드 `8VTJS2`)과 "테스트학생"의 3라운드 제출물. 지우지 않고 남기기로 사용자가 결정했다.

### 남은 것 / 관찰

- **카드 높이가 행마다 다르다.** 보완단원 설명이 3줄(다른 단원은 2줄)이라 2행 카드가 244px, 1행은 225px이다. 그리드가 행 높이를 맞추느라 6단원 카드까지 함께 커졌다. 레이아웃이 깨진 건 아니고, 설명을 31자 이하로 줄이면 통일된다
- `travel-places-only`에서 **미션 재시도가 4회**까지 갔다(한계 5회). 글이 짧고 빈약해 모델이 없는 내용을 인용하려 한 것으로 보인다
- 같은 케이스에서 인용 치환이 *"학생 글의 관련 내용은 생각과 실제 본 모습을 비교하며…"* 라는 읽기 힘든 문장을 만들었다. 치환 자체는 의도대로 동작했고 학생에게 틀린 말이 나가지는 않았다
- `ACTION_TITLE_PATTERN` 화이트리스트는 **건드리지 않았다** — 기행문 미션 제목이 전부 기존 동사(`다듬기`·`덧붙이기`·`쓰기`)로 나왔다
