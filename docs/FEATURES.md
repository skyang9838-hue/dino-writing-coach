...
- **면담 보고서 파일럿이 라이브 평가 없이 프로덕션에 올라가 있다** — 2026-08-02 배포(`b30e060`)로 프로덕션에 나갔지만, 프롬프트 품질 게이트(`npm run eval`)는 여전히 통과한 적이 없다. 보드/파이프라인 **동작**은 실제 코칭 7라운드로 검증됐으나 판정 **품질**은 미검증이다.
