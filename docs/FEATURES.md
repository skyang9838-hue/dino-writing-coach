...
- **6학년 2학기 국어 단원 선택**(`lib/curriculum.js`의 `GRADE6_SEMESTER2_UNITS`) — 1·2·5·6단원과 **매체 단원** 중 아이콘이 붙은 카드로 선택. 매체 단원은 교육과정에 번호가 없어 카드의 번호 자리에 `unitLabel`(`'매체 단원'`)이 대신 들어가고, 목록에서는 5단원과 6단원 사이에 놓인다. 단원마다 장르(`genre`)와 권장 글자 수가 미리 정해져 있어, 카드를 고르면 목표 글자 수가 그 값으로 자동 이동한다. 2026-08-31에 1학기 목업 8개에서 2학기 실제 단원 5개로 교체됐다.
