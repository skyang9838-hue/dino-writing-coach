...
- **라운드 카드**(`components/RevisionBoard.jsx`, Server Component): 가로로 나란히 배치되고 카드 사이에 `›`가 들어간다. 카드 폭 325px 고정으로 `.container-widest`(1440px) 안에 **4개가 스크롤 없이** 들어오고, 더 많으면 가로 스크롤(`tabIndex={0}`이라 키보드로도 스크롤됨). 카드 한 장의 구성:
  ...
  - **📋 AI가 본 채점기준** — 면담 보고서일 때만. `lib/curriculum.js`의 기준 7개를 루브릭 순서대로 놓고 `round.assessments`의 판정을 `✓`(met, "충족으로 보임") `?`(partial, "판단이 애매함") `✕`(unmet, "확인 안 됨")로, 직전 라운드 같은 기준과 비교한 변화를 `–`(변화 없음) `↑`(향상) `▼`(하락)로 보여준다. 첫 카드는 변화 칸이 빈다. 표에 들어갈 짧은 라벨은 기준마다 `shortLabel`로 추가됨(프롬프트가 쓰는 `label`은 그대로).
  - **기호는 확신을 덜 주는 쪽으로 일부러 골랐다** — 원래 `○ 충분히 충족 / △ 보완 필요 / ✕ 미충족`이었으나, 이건 **사람 교사가 확정 채점할 때 쓰는 표기**인데 실제로 판정한 건 AI다. 같은 글을 두 번 판정했을 때 결과가 갈리는 것이 2026-08-02 실측으로 확인됐다(6-1 참고). 표 제목도 `채점기준표`에서 `AI가 본 채점기준`으로 바꾸고 그 아래 "선생님 확정 채점이 아니에요"를 붙였다. 옛 기호와 확정 채점 어투가 되돌아오는 걸 `components/infiniteCoachingUi.test.js`가 소스에서 막는다.
...
- **하단 안내 바**: AI 판정 표시 / 변화 표시 / 수정 미션 / 글 표시(diff) 범례. 변화 표시 묶음에는 "지난 회차의 AI 판정과 견준 것이에요"가 붙는다. 채점기준표 관련 두 묶음은 루브릭이 있는 장르에서만 나온다.
