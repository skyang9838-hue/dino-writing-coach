...
- **6학년 2학기 국어 단원 선택**(`lib/curriculum.js`의 `GRADE6_SEMESTER2_UNITS`) — 1·2·5·6단원과 **매체 단원**, **보완단원** 중 아이콘이 붙은 카드로 선택. 이 둘은 교육과정에 번호가 없어 카드의 번호 자리에 `unitLabel`(`'매체 단원'` / `'보완단원'`)이 대신 들어간다. 매체 단원은 5단원과 6단원 사이, 보완단원은 목록 맨 뒤에 놓인다. 단원마다 장르(`genre`)와 권장 글자 수가 미리 정해져 있어, 카드를 고르면 목표 글자 수가 그 값으로 자동 이동한다. 2026-08-31에 1학기 목업 8개에서 2학기 실제 단원 5개로 교체됐고, 2026-09-01에 보완단원이 더해져 6개가 됐다.
...
| 매체 단원 매체 성찰 보고서 쓰기 | 6국06-04, 6국03-06 | 매체 이용 양상 성찰 / 쓰기에 적극 참여 / 공유하는 태도 | 3 / 0 / 2 |
| 5단원 기사문 작성하기 | 6국03-04 | 독자 고려 내용 생성 / 매체 고려 표현 | 2 / 3 / 1 |
| 6단원 이야기 바꾸어 쓰기 | 6국05-05 | (청크 없음) | 3 / 0 / 3 |
| 보완단원 기행문 쓰기 | 6국03-03, 6국03-05 | 기행문의 특성 / 쓰기 과정과 고쳐쓰기 | 5 / 1 / 2 |

**보완단원**은 성취기준에는 있으나 교과서에 단원이 없는 자리를 메운다(2026-09-01 추가). 뒤쪽 청크(`6국03-05` 쓰기 과정과 고쳐쓰기)는 항목 둘이 모두 `teacher`라 **청크째로 AI에게 가지 않는다** — 쓰기 과정을 점검했는지도, 고쳐쓰기로 무엇이 달라졌는지도 완성된 글 하나만 봐서는 알 수 없기 때문이다. 보드에는 그대로 나온다.
...
### 스모크 러너 (`npm run smoke`, `scripts/smoke-rubric-coaching.js`, `evals/smoke/`)

새 단원을 넣었을 때 **제일 먼저 돌리는 것.** 정답표 없이 "이 단원이 돌기는 도는가, 판정이 말이 되는가"만 본다. 아래 평가 루프와는 다른 물건이다 — 그쪽은 정답표를 두고 일치율로 게이트를 건다.

단원마다 `evals/smoke/<unitId>.json`에 잘 쓴 글 / 중간 / 부족한 글 3편이 있고, 케이스마다 각 기준을 어떤 판정으로 의도했는지(`intended`)와 무엇을 일부러 넣고 뺐는지(`note`)가 붙어 있다. `getUnitCoachingSpec` + `getRubricCoachingFeedback`를 직접 부르므로 **DB를 건드리지 않는다** — `.env.local`의 `DATABASE_URL`이 프로덕션 Neon을 가리키기 때문에 이게 중요하다.
...
사람이 안 봐도 되는 실패는 자동으로 걸러 낸다: `evaluator: 'teacher'` 기준이 응답에 섞였는지(**방화벽이 실제로 막는지** — 유닛 테스트는 프롬프트에 안 들어가는 것까지만 본다), 조언용 기준이 판정에서 빠지지 않았는지, 미션이 정확히 2개인지, 재시도를 몇 번 했는지. 결과는 `.smoke-results/`에 남는다.

**글도 의도한 판정도 저장소가 지어낸 것이다.** 여기서 맞는다는 건 "모델이 글을 의도대로 읽었다"는 뜻이지 선생님이 검증한 정답과 맞다는 뜻이 아니다. 실제 학생 글이 생기면 그걸로 다시 돌려야 한다.
