...
**마지막 갱신:** 2026-07-10 (세션 종료)
**최신 커밋:** `f6aa96a` docs: diff 강조 기능 문서 반영
...
**기타 — 글쓰기 박스 크기 확대**
- 학생용 글쓰기 textarea 높이를 200px → 400px → 600px로 확대 (교실 사용 시 너무 작다는 피드백 반영).
...
- 라운드별로 "초안 / N차 수정" 라벨, 그때 제출한 글 전문, 직전 미션(보완점) 반영 여부 ✅/❌, 새로 받은 코칭 내용을 카드로 표시. 기본은 접힌 상태, 토글로 펼침.
- 도달도 게이지 옆에 `N회차` 표시 추가.
- 서버/데이터 모델 변경 없음 — 기존 저장 데이터를 그대로 읽어서 렌더링만 함.
- Playwright로 3라운드 전체 흐름(도달도 40→50→70, 미션 반영 체크 표시) 검증 완료.

**7단계 — 글쓰기 세션 자동 저장 / 새로고침 복구**
- `src/sessionStorage.js` 신규: localStorage에 `topic`/`writing`/`feedback`/`attainment`/`rounds` 등 세션 전체를 자동 저장.
- 앱 재로드 시 저장된 값 복구 — 새로고침해도 글, 도달도, 마지막 코칭 결과 유지됨.
- `rounds`는 코칭마다 누적되는 배열로 설계해 8단계(퇴고 히스토리)에서 그대로 재사용.
- "새 글 시작" 버튼 추가 — 확인창 후 저장된 내용 전부 삭제(교실 공유 컴퓨터에서 다음 학생이 이전 학생 글을 보지 않도록).
- Playwright로 새로고침 후 복구, 리셋 후 재확인 검증 완료.
...
- **해결된 이슈:** 초기엔 Gemini 무료 등급의 짧은 시간당 요청 제한(20회)에 걸려 간헐적으로 502가 발생했음. Google Cloud 결제 계정을 연결해 유료(종량제) 등급으로 전환하여 해결 확인(연속 8회 요청 모두 성공).
...
1. **디노 캐릭터 이미지/애니메이션 적용** — (5단계에서 `hero.png`, `icons.svg`는 미사용 상태로 삭제됨, 다시 도입 시 새로 준비 필요)
2. **라우팅** — 화면이 여러 개로 늘어날 경우에만 필요 (현재는 불필요할 수 있음)
3. **(보류 중) 교사/학생 역할 분리** — 퇴고 히스토리의 diff 강조를 교사에게만 보여줄지 논의했으나, 로그인/역할 시스템 자체가 없어서 의도적으로 보류함. 실사용 후 필요성이 확인되면 재검토 (자세한 내용은 `FEATURES.md` 13번 항목 참고)
...
오늘은 [디노 캐릭터 적용 / 라우팅 / 교사·학생 역할 분리 검토] 중에서
