...
**마지막 갱신:** 2026-07-11 (master 병합 + Vercel 배포 전환 + Phase 4 부분 구현 — 무의미한 글/스페이스 도배 감지 가드)
**최신 커밋:** `d98d95a` chore: wire prisma migrate deploy into Vercel build, pin framework to nextjs (가드 관련 커밋은 아직 미완료 — 다음 세션 시작 시 `git status`로 확인)
**GitHub 저장소:** https://github.com/skyang9838-hue/dino-writing-coach (public)
**배포 주소:** https://dino-writing-coach.vercel.app (Next.js 교실 플랫폼 버전이 실제로 배포되어 있음 — 기존 Vite 버전은 더 이상 배포되지 않음)
...
## Phase 4 (부분) — 무의미한 글 / 스페이스 도배 감지 가드

PRD 로드맵의 Phase 4 중 "무의미한 글(자모 반복/스페이스 도배)" 부분만 구현. **욕설/비속어 감지 + 교사 승인(O/X) 큐는 이번엔 범위에서 제외**하고 다음으로 미룸(아래 로드맵 3번 참고 — 설계를 논의하다 "이번엔 너무 깊다"는 판단으로 보류).
...
## Phase 2 이후 로드맵 (다음에 할 일)

1. **Phase 4 나머지 — 욕설/비속어 감지 + 교사 승인 큐**: 규칙 기반 단어 리스트로 1차 감지 → 자동으로 도달도 0 처리하는 대신 **교사가 나중에 O/X로 검토**하는 방식으로 하기로 논의함. 아직 정하지 못한 것: 교사가 검토 전 학생 화면(다음 코칭 요청 차단은 정함), 검토 위치(활동 상세 페이지 vs 학생별 페이지), 승인(O) 시 그제서야 실제 Gemini 호출해서 정상 코칭 진행(정함) — 다음 세션에서 이어서 설계 필요.
2. **Phase 2 잔여**: 참여 학생 목록 정렬/검색(필요해지면 별도 요청 — "제출 현황" 필터는 제출 개념 자체가 없어져서 더 이상 해당 없음)
3. **Phase 3 — 루브릭**: 표/이미지 업로드 + AI 파싱 + 충족 여부 체크(O/X만, 점수 없음), 교사가 고르는 피드백 우선순위(내용/구조/표현력 등)가 코칭 프롬프트에 반영
...
```
dino-writing-coach 프로젝트를 이어서 작업합니다 (master 브랜치, 이미 프로덕션 배포됨).
docs/PROJECT_STATUS.md에서 현재 상태와 로드맵을 확인해줘.
master 병합 + Vercel 배포 전환 완료, Phase 4 중 무의미한 글/스페이스 도배 감지 가드까지 완료됐어.
이제 [Phase 4 나머지(욕설/비속어 + 교사 승인 큐 설계) / Phase 2 잔여(목록 정렬·검색) / Phase 3(루브릭)] 중에서
